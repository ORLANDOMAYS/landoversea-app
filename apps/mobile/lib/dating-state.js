function normalizeDiscoveryFilters(filters = {}) {
  const numberInRange = (value, fallback) => {
    ...
    return Number.isFinite(parsed) ? Math.min(120, Math.max(18, Math.trunc(parsed))) : fallback;
  };
  const minAge = numberInRange(filters.minAge, 18);
  const maxAge = Math.max(minAge, numberInRange(filters.maxAge, 120));
  return {
    minAge,
    maxAge,
    gender: typeof filters.gender === "string" ? filters.gender.trim() : "",
    location: typeof filters.location === "string"
